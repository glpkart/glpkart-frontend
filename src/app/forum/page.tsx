...
export default function ForumPage() {
  ...
  // FIX #14: mounted gate prevents flash — nothing renders until after
  // localStorage has been hydrated into the store (client-side only)
  ...
  useEffect(() => {
    // Mark as mounted — now store has hydrated from localStorage
    setMounted(true)
  }, [])
  ...
  const Nav = (
    <nav style={{ background: 'white', borderBottom: `1px solid ${BORDER}`, padding: '0 32px', height: 58, display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontFamily: SANS }}>
      ...
      <button onClick={() => { logout(); router.push('/') }} style={{ fontSize: 13, color: FAINT, background: 'none', border: 'none', cursor: 'pointer', fontFamily: SANS }}>
        Sign out
      </button>
    </nav>
  )

  // FIX #14: While not yet mounted show a neutral blank shell — no flash
  if (!mounted) return (
  ...
  if (isLoggedIn() && !canAccessForum()) return (
    <div style={{ minHeight: '100vh', background: '#FAFAF7', fontFamily: SANS }}>
      {Nav}
      <div style={{ maxWidth: 600, margin: '80px auto', padding: '0 24px', textAlign: 'center' }}>
        <div style={{ width: 64, height: 64, borderRadius: '50%', background: GL, margin: '0 auto 24px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <svg width="28" height="28" viewBox="0 0 28 28" fill="none">
            <rect x="4" y="13" width="20" height="13" rx="2" stroke={G} strokeWidth="1.5"/>
            <path d="M9 13V9a5 5 0 0110 0v4" stroke={G} strokeWidth="1.5" strokeLinecap="round"/>
          </svg>
        </div>
        <h2 style={{ fontFamily: SERIF, fontSize: 28, fontWeight: 700, color: INK, marginBottom: 12 }}>
          Community unlocks after treatment starts
        </h2>
        <p style={{ fontSize: 15, color: MUTED, lineHeight: 1.75, marginBottom: 32 }}>
          The forum is available to patients who have completed their first consultation. This keeps the community genuine — real experiences from people actually on treatment.
        </p>
        <div style={{ background: GL, border: '1px solid #C0DD97', borderRadius: 14, padding: 24, marginBottom: 24, textAlign: 'left' }}>
          ...
          {['Request a consultation', 'Complete your WhatsApp call with the doctor', 'Receive your prescription', 'Forum access granted automatically'].map((step, i) => (
        ...
        <p style={{ fontSize: 12, color: FAINT, marginTop: 12 }}>₹799 · Full refund if not eligible</p>
      </div>
    </div>
  )

  // UNLOCKED — logged in with completed consultation
  return (
    <div style={{ minHeight: '100vh', background: '#FAFAF7', fontFamily: SANS }}>
      {Nav}
      <div style={{ maxWidth: 780, margin: '0 auto', padding: '36px 24px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
          <div>
            <h1 style={{ fontFamily: SERIF, fontSize: 32, fontWeight: 700, color: INK, marginBottom: 4 }}>Community</h1>
            <p style={{ fontSize: 14, color: MUTED }}>Anonymous, honest conversations about GLP-1. Your real name is never shown.</p>
          </div>
          <button
            onClick={() => setShowNew(true)}
            style={{ padding: '9px 20px', background: G, color: 'white', borderRadius: 8, border: 'none', fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: SANS, marginTop: 4 }}
          >
            New post
          </button>
        </div>
        ...
        <div style={{ background: GL, border: '1px solid #C0DD97', borderRadius: 10, padding: '10px 16px', marginBottom: 24, fontSize: 13, color: '#15803c', display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          Fully anonymous. Your posts appear as <strong style={{ marginLeft: 4 }}>{user?.forumPersona || 'your persona'}</strong>. Your real name or phone is never shown.
        </div>
        ...
        {showNew && (
          <div style={{ background: 'white', borderRadius: 14, padding: 28, border: `1px solid ${BORDER}`, marginBottom: 24 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
              <h3 style={{ fontSize: 16, fontWeight: 600, color: INK }}>Share your experience</h3>
            ...
            <select value={newTopic} onChange={e => setNewTopic(e.target.value)}
              style={{ width: '100%', padding: '10px 14px', border: `1.5px solid ${BORDER}`, borderRadius: 8, fontSize: 14, marginBottom: 12, outline: 'none', fontFamily: SANS }}>
              {TOPICS.filter(t => t !== 'All').map(t => <option key={t} value={t}>{TOPIC_LABELS[t]}</option>)}
            </select>
            <input type="text" placeholder="Title" value={newTitle} onChange={e => setNewTitle(e.target.value)}
              style={{ width: '100%', padding: '12px 14px', border: `1.5px solid ${BORDER}`, borderRadius: 8, fontSize: 14, marginBottom: 12, outline: 'none', fontFamily: SANS }} />
            <textarea placeholder="Share your experience, question, or story..." value={newBody} onChange={e => setNewBody(e.target.value)} rows={4}
              style={{ width: '100%', padding: '12px 14px', border: `1.5px solid ${BORDER}`, borderRadius: 8, fontSize: 14, marginBottom: 12, outline: 'none', fontFamily: SANS, resize: 'vertical' }} />
            {error && <p style={{ color: '#ef4444', fontSize: 13, marginBottom: 10 }}>{error}</p>}
            <button onClick={submitPost} disabled={submitting}
              style={{ padding: '10px 24px', background: G, color: 'white', borderRadius: 8, border: 'none', fontSize: 14, fontWeight: 500, cursor: 'pointer', fontFamily: SANS }}>
        ...
        {/* Posts list */}
        {loading ? (
          <div style={{ textAlign: 'center', padding: '60px 0', color: FAINT, fontSize: 15 }}>Loading posts…</div>
        ) : posts.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 0', color: FAINT, fontSize: 15 }}>
            ...
            {posts.map(post => (
              <div key={post.id} style={{ background: 'white', borderRadius: 14, padding: '20px 22px', border: `1px solid ${BORDER}` }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                  <div style={{ width: 34, height: 34, borderRadius: '50%', background: GL, border: '2px solid #86efac', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, color: '#3B6D11', flexShrink: 0 }}>
                    {post.personaName[0]}
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: INK }}>{post.personaName}</div>
                    <div style={{ fontSize: 11, color: FAINT }}>{timeAgo(post.createdAt)}</div>
                  </div>
                  {post.isDoctorPost && (
                    <span style={{ fontSize: 10, background: '#dcfce9', color: '#15803c', padding: '2px 9px', borderRadius: 999, fontWeight: 700 }}>Doctor</span>
                  )}
                  <span style={{ fontSize: 10, padding: '2px 9px', borderRadius: 999, background: TOPIC_COLORS[post.topic] || '#f3f4f6', color: '#374151', fontWeight: 500 }}>
                    {TOPIC_LABELS[post.topic] || post.topic}
                  </span>
                </div>
                <h3 style={{ fontSize: 16, fontWeight: 600, color: INK, marginBottom: 6 }}>{post.title}</h3>
                ...
                <div style={{ display: 'flex', gap: 16, fontSize: 12, color: FAINT }}>
                  <button onClick={() => markHelpful(post.id)}
                    style={{ background: 'none', border: 'none', color: FAINT, cursor: 'pointer', fontSize: 12, fontFamily: SANS, padding: 0 }}>
                    ♥ {post.helpfulCount} helpful
                  </button>
                  <Link href={'/forum/' + post.id} style={{ color: FAINT, textDecoration: 'none' }}>
