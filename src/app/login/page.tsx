        ...
        {step === 'otp' && (
          <div>
            ...
            <input
              ...
              style={{ width: '100%', padding: '16px', border: '2px solid #e5e7eb', borderRadius: 10, fontSize: 28, textAlign: 'center', outline: 'none', fontFamily: 'inherit', letterSpacing: '0.3em', marginBottom: 12 }}
            />
            {error && <p style={{ color: '#ef4444', fontSize: 13, marginBottom: 12 }}>{error}</p>}
            <button onClick={verifyOtp} disabled={loading || otp.length !== 6} style={btnStyle}>
              {loading ? 'Verifying...' : 'Verify OTP →'}
            </button>
            <button onClick={() => { setStep('phone'); setOtp(''); setError('') }} style={{ display: 'block', width: '100%', marginTop: 12, padding: '10px', background: 'none', border: 'none', color: '#6b7280', fontSize: 14, cursor: 'pointer', fontFamily: 'inherit' }}>
              ← Change number
            </button>
          </div>
        )}
